...
Глубокий путь может быть легален внутри того же слайса — файл в `entities/user/ui/` импортирует `../model/types`. Это grep по `@/` не ловит, потому что алиас не используется внутри слайса.

`@x`-фасады вида `from '@/entities/user/@x/post'` скрипт **не флагирует**: regex требует сегмент из `(ui|model|api|lib|config)`, а `@x` в этот whitelist не попадает. Это by design.

FP возможно, если слайс или сегмент назван не kebab-case (паттерн `[a-z0-9-]+`). Для legacy-имён во время миграции — расширьте regex.
